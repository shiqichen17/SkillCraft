# Task: D&D Monster Compendium (5 Monsters × 5 APIs) - H1

Create monster entries for **5 iconic D&D monsters** using 5 API endpoints per monster.

## Objective

For each of the following **5 monsters**, collect:
1. **Monster Info**: HP, AC, CR, abilities
2. **Spell Info**: spell details
3. **Class Info**: class data
4. **Race Info**: race data
5. **Equipment**: gear categories
...
```json
{
  ...
  "summary": {"total_monsters": 5, "total_xp": 50000},
...
For each monster (Adult Red Dragon, Kraken, Aboleth, Lich, Tarrasque):
1. `local-dnd_get_monster`
2. `local-dnd_get_spell`
3. `local-dnd_get_class`
4. `local-dnd_get_race`
5. `local-dnd_get_equipment_category`
...
1. Process ALL 5 monsters completely
