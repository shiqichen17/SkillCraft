# Task: TV Series Analysis (4 Shows × 4 APIs) - M2

Perform analysis of **4 TV series** using 4 API endpoints per show.

## Objective

For each of the following **4 TV shows**, collect:
1. **Show Info**: Name, rating, genres, status, network
2. **Episode Summary**: Total count, avg rating, highest rated
3. **Cast Summary**: Total count, lead actors
4. **Crew Summary**: Creator, director/writer counts
  ...
  "shows": [
    {
      "id": 169,
      "name": "Breaking Bad",
      "rating": 9.5, "status": "Ended", "genres": ["Drama"], "network": "AMC", "episodes": {"total_count": 62, "avg_rating": 9.2, "highest_rated": {"name": "Ozymandias", "rating": 10.0}}, "cast": {"total_count": 50, "leads": [{"actor": "Bryan Cranston", "character": "Walter White"}]}, "crew": {"creator": "Vince Gilligan", "directors_count": 15},
      "marathon_hours": 46.5,
      "binge_rating": "A+"
    }
  ],
...
For each of the 4 shows (IDs: 169, 82, 526, 431):
1. `local-tvmaze_get_show_info`
2. `local-tvmaze_get_show_episodes`
3. `local-tvmaze_get_show_cast`
4. `local-tvmaze_get_show_crew`
...
1. Process ALL 4 shows completely - do NOT use placeholder data
...
1. **FIRST**: Collect all data for all 4 shows
