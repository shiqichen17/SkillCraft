#!/usr/bin/env python3
"""
Evaluation script for tvmaze-series-analyzer-h1 task.
5 shows × 5 API calls = 25 total calls.
"""
import json
import os
import sys
...
KNOWN_DATA = {
    169: {"name": "Breaking Bad", "min_rating": 9.0, "max_rating": 10.0, "min_episodes": 60, "max_episodes": 65},82: {"name": "Game of Thrones", "min_rating": 8.5, "max_rating": 10.0, "min_episodes": 70, "max_episodes": 75},526: {"name": "The Office", "min_rating": 8.0, "max_rating": 9.5, "min_episodes": 180, "max_episodes": 210},431: {"name": "Friends", "min_rating": 8.0, "max_rating": 9.5, "min_episodes": 230, "max_episodes": 240},2993: {"name": "Stranger Things", "min_rating": 8.0, "max_rating": 9.5, "min_episodes": 30, "max_episodes": 40}
}

REQUIRED_FIELDS = ['id', 'name', 'marathon_hours', 'binge_rating', 'rating', 'seasons', 'episodes', 'cast', 'crew']
...
def validate_summary_format(show: Dict) -> tuple:
    ...
    if isinstance(episodes, dict):
        if episodes.get('total_count') is not None or episodes.get('total') is not None:
            score += 1
        total += 1
...
def evaluate(workspace: str, groundtruth_dir: str) -> Dict[str, Any]:
    score = EvalScore("tvmaze-series-analyzer-h1")
