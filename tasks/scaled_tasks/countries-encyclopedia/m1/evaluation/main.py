#!/usr/bin/env python3
"""
Evaluation script for countries-encyclopedia-m1 task (Medium: 4 regions × 4 API calls).
Tools: region, details, borders, currency
...
def validate_region_data(region: Dict) -> Tuple[bool, List[str]]:
    ...
    if not region.get("currency_analysis"):
...
def evaluate(workspace: str, groundtruth_dir: str) -> Dict[str, Any]:
    score = EvalScore("countries-encyclopedia-m1")
...
def main():
    parser = argparse.ArgumentParser(description='Evaluate countries-encyclopedia-m1 task')
