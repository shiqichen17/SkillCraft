#!/usr/bin/env python3
"""
Evaluation script for countries-encyclopedia-e1 task (Easy: 3 regions × 3 API calls).
Tools: region, details, borders
...
8. Global summary validation (15 points)
"""
import json
import os
import sys
...
EXPECTED_REGIONS = ["europe", "asia", "americas"]
...
def validate_region_data(region: Dict) -> Tuple[bool, List[str]]:
    ...
    borders = region.get("border_analysis", {})
    if not borders:
...
def evaluate(workspace: str, groundtruth_dir: str) -> Dict[str, Any]:
    score = EvalScore("countries-encyclopedia-e1")
    ...
    if summary.get("total_regions") == actual_count:
        summary_checks += 1
    if summary.get("total_countries", 0) > 50:
        summary_checks += 1
    if summary.get("largest_region"):
        summary_checks += 1
    if summary.get("total_world_population", 0) > 1_000_000_000:
        summary_checks += 1
...
def main():
    parser = argparse.ArgumentParser(description='Evaluate countries-encyclopedia-e1 task')
