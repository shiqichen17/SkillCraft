# Task: Earthquake Analysis (5 Regions × 5 APIs) - H1

Analyze earthquake activity in **5 seismically active regions** using 5 API endpoints per region.

## Objective

For each of the following **5 regions**, collect:
1. **Query Earthquakes**: get recent earthquakes
2. **Region Stats**: get statistical analysis
3. **Historical Trends**: 90-day trends
4. **Depth Analysis**: depth distribution
5. **Tectonic Info**: plate boundary info
...
| 5 | Istanbul, Turkey | 41.0082 | 28.9784 |
...
```json
{
  ...
  "summary": {"total_regions": 5, "total_earthquakes": 500, "most_active_region": {"name": "Tokyo, Japan"}},
...
For each region (Tokyo, Los Angeles, Jakarta, Santiago, Istanbul):
1. `local-usgs_query_earthquakes`
2. `local-usgs_get_region_stats`
3. `local-usgs_get_historical_trends`
4. `local-usgs_get_depth_analysis`
5. `local-usgs_get_tectonic_info`
...
1. Process ALL 5 regions completely
