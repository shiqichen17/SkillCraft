#!/usr/bin/env python3
"""
Evaluation script for gitlab-deep-analysis-m2 task (Medium: 4 projects × 4 API calls).
Tools: project_info, contributors, branches, issues
...
REQUIRED_FIELDS = ['path', 'name', 'info', 'contributors', 'branches', 'issues', 'health_score']
...
def validate_project_data(project: Dict) -> bool:
    ...
    if info is not None and isinstance(info, dict):
        if info.get('stars') is None and info.get('star_count') is None:
            return False
...
def evaluate(workspace: str, groundtruth_dir: str) -> Dict[str, Any]:
    score = EvalScore("gitlab-deep-analysis-m2")
    result_file = os.path.join(workspace, "gitlab_comprehensive.json")
    expected_count = len(EXPECTED_PROJECTS)
    
    if not os.path.exists(result_file):
        score.add("output_file_exists", 5, 0, "gitlab_comprehensive.json not found")
...
def main():
    parser = argparse.ArgumentParser(description='Evaluate gitlab-deep-analysis-m2 task')
