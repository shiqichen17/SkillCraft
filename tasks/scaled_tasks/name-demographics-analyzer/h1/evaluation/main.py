#!/usr/bin/env python3
"""Evaluation for name-demographics-analyzer-h1: 5 names × 5 APIs"""
...
EXPECTED_NAMES = ['michael', 'yuki', 'hans', 'maria', 'chen']
EXPECTED_COUNT = 5
REQUIRED_FIELDS = ['name', 'profile', 'gender', 'age', 'nationality']
...
def validate_name_entry(entry):
    ...
    if entry.get('age'): score += 0.2
    if entry.get('nationality'): score += 0.2
    if entry.get("profile"): score += 0.2
    return score, []

def evaluate(workspace, groundtruth_dir):
    score = EvalScore("name-demographics-analyzer-h1")
